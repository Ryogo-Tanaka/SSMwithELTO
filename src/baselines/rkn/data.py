"""quad-link image dataset for the RKN baseline.

Mirrors `src/baselines/lstm/data.py` exactly (window-based sliding dataset over
the single training trajectory). The RKN cell expects per-step image inputs of
shape (B, T, H, W, C) which the encoder then permutes to (B*T, C, H, W) for the
...
class QuadlinkRknDataset(Dataset):
