#!/usr/bin/env python3
"""Multi-step rollout evaluation for the RKN baseline.

Mirrors `evaluate_multistep_rollout_lstm.py`. Rollout protocol (v3):
    s = 0 fixed, single rollout per (regime, seed, C).
    Context y[0..C-1] -> free rollout of H_max frames via RKNCell._predict
    iterated without observation update.
    For each h in horizons, target = y[C + h - 1], pred = rollout output [h-1].

Usage:
    python scripts/rebuttal/evaluate_multistep_rollout_rkn.py \
        --ckpt results/rebuttal/rkn/clean/seed1/ckpts/model_final.pt \
        --regime clean --data_seed 1 --seed_label 1 \
        --ctx_len 5 \
        --horizons 1,5,10,20,50 \
        --output results/rebuttal/rkn/clean/seed1/rollout/ctx5
"""
