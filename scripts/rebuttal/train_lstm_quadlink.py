#!/usr/bin/env python3
"""Train CNN-LSTM-CNN baseline on quad-link images (PROJ-REBUTTAL Step 8).

Mirrors the structure of `train_ncdssm_quadlink.py` but for the LSTM
baseline. Outputs:
    {output}/
        logs/
            config.yaml          (resolved config snapshot)
            train_metrics.jsonl  (per-step / per-epoch JSON lines)
        ckpts/
            model_epoch_{N}.pt
            model_final.pt

Usage:
    PYTHONUNBUFFERED=1 python scripts/rebuttal/train_lstm_quadlink.py \
        --regime clean --data_seed 1 \
        --output results/rebuttal/lstm/clean/seed1 \
        --num_epochs 30
"""
...
from src.baselines.lstm.data import QuadlinkLstmDataset, resolve_quadlink_path


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--config", type=str,
                   default=str(REPO_ROOT / "configs" / "rebuttal" / "lstm_quadlink.yaml"))
...
def compute_loss(model, batch_y: torch.Tensor) -> torch.Tensor:
    """Teacher-forcing next-step MSE loss.

    batch_y: (B, T, H, W, C). y_pred[:, t] is the model's prediction of y[:, t+1].
    The last prediction has no ground-truth target, so it is dropped.
    """
    y_pred = model(batch_y)  # (B, T, H, W, C)
    pred = y_pred[:, :-1]
    target = batch_y[:, 1:]
