#!/usr/bin/env python3
"""Evaluate the LSTM baseline on quad-link 1-step prediction (DSE-aligned).

Mirrors `evaluate_ncdssm_quadlink.py`. The LSTM model produces predicted
features for time t+1 from inputs y[:t+1]. For DSE-aligned protocol the
target is y[s + ctx_len + 1] (skip 1 frame), so we run a free 2-step rollout
from the context window and use the second prediction.

Usage:
    python scripts/rebuttal/evaluate_lstm_quadlink.py \
        --ckpt results/rebuttal/lstm/clean/seed1/ckpts/model_final.pt \
        --regime clean --data_seed 1 \
        --output results/rebuttal/lstm/clean/seed1/eval_final
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--protocol", type=str, choices=("dse", "onestep"),
                   default="dse",
                   help="dse=skip 1 frame (input y[s..s+L-1] -> target y[s+L+1]); "
                        "onestep=standard next-step (target y[s+L]).")
...
def main() -> int:
    ...
    if args.protocol == "dse":
        # input y[s..s+L-1] -> target y[s+L+1]; horizon = 2 (take prediction 2)
        rollout_h = 2
        target_offset = L + 1
        first_s = 0
        last_s = T - L - 2  # need s+L+1 <= T-1
    ...
    # Build context windows tensor (N, L, H, W, C).
    ...
    with torch.no_grad():
        for i in range(0, N, bsz):
            ...
            pred_target = preds[:, -1]  # take final-step prediction
